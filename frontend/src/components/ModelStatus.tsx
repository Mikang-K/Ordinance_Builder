...
export default function ModelStatus({ instanceId = 'model-status' }: { instanceId?: string }) {
  ...
  return <div className="model-status" ref={root}>
    <button type="button" className={`model-status-trigger ${overall}`} aria-expanded={open} aria-controls={`${instanceId}-panel`} onClick={() => setOpen(v => !v)}><span className="model-status-dot" aria-hidden="true" /><span>{failed ? '모델 상태 확인 불가' : data ? `AI 모델 ${label(data.status)}` : 'AI 모델 확인 중'}</span></button>
    {open && <section className="model-status-panel" id={`${instanceId}-panel`} aria-label="활성 AI 모델 구성">
